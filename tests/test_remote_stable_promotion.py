...
class RemoteStablePromotionTests(unittest.TestCase):
    ...
    def test_remote_validation_script_uses_real_repo_file(self):
        ...
        self.assertIn('target_version="$(rpm -qp --qf \'%{VERSION}\' "$target_rpm")"', text)
        self.assertIn('target_release="$(rpm -qp --qf \'%{RELEASE}\' "$target_rpm")"', text)
        self.assertNotIn("0.2.4-1.fc44", text)
        self.assertNotIn("grep -F '0.2.4'", text)
